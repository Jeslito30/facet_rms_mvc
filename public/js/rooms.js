...
const BOOKINGS_API_ENDPOINT = '/facet-rms/public/booking';
...
if (bookRoomForm) {
  bookRoomForm.addEventListener("submit", async (e) => {
    ...
    try {
        ...
        const response = await fetch(`${BOOKINGS_API_ENDPOINT}/create_api`, {
            method: 'POST',
...
const API_ENDPOINT = '/facet-rms/public/room'
/**
 * Fetches all rooms from the PHP API endpoint. (READ operation)
 */
async function loadRoomsFromDB() {
  try {
    console.log('Loading rooms from API:', `${API_ENDPOINT}/get_all_api`);
    const response = await fetch(`${API_ENDPOINT}/get_all_api`, {
        method: 'GET',
        headers: {
...
async function saveRoomToDB(roomData) {
  try {
    const response = await fetch(`${API_ENDPOINT}/save_room_api`, {
...
async function deleteRoomFromDB(roomId, roomName) {
  try {
    const response = await fetch(`${API_ENDPOINT}/delete_room_api`, {
      method: 'POST',
      headers: {
