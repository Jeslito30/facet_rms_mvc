...
async function loadTeachers() {
    try {
        ...
        const response = await fetch(`${API_ENDPOINT}/list_api?${params}`);
...
function renderTeachers(teachersToRender) {
    ...
    teachersToRender.forEach((teacher) => {
        ...
        const profilePicUrl = teacher.profile_picture_type 
            ? `${API_ENDPOINT}/picture_api/${teacher.id}` 
            : null;
...
window.viewTeacher = async (id) => {
    try {
        const response = await fetch(`${API_ENDPOINT}/get_api/${id}`);
        const result = await response.json();
        
        if (result.success) {
            // Store user data for details page
            localStorage.setItem("viewUserData", JSON.stringify(result.data));
            window.location.href = `/facet-rms/public/teacher/details/${id}`;
        } else {
...
window.deleteTeacher = async (id, name) => {
    ...
    try {
        const response = await fetch(`${API_ENDPOINT}/delete_api`, {
            method: 'POST',
...
teacherForm.addEventListener("submit", async (e) => {
    ...
    try {
        const action = editingTeacherId ? 'update_api' : 'create_api';
        if (editingTeacherId) {
